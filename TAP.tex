...
As required by \citep{std:DALI11}, all resources except the VOSI-availability must 
...
DALI-sync \citep{std:DALI11} resources. The exact form of the query, and hence the 
...
for DALI-async \citep{std:DALI11} resources and thus represent a job-list 
...
The use of the VOSI-availability resource is described in \citep{std:DALI11}.
...
The use of the VOSI-capabilities resource is described in \citep{std:DALI11}.
...
<vosi:capabilities xmlns=""
  ...
  <vosi:capability standardID="ivo://ivoa.net/std/TAP#async-1.1">
    ...
    <interface xsi:type="vod:ParamHTTP" role="std" version="1.1">
      ...
      <securityMethod 
        standardID="http://www.w3.org/Protocols/HTTP/1.0/spec.html#BasicAA"/>
    </interface>
    <interface xsi:type="vod:ParamHTTP" role="std" version="1.1">
  ...
  <vosi:capability standardID="ivo://ivoa.net/std/VOSI#tables">
    ...
    <interface xsi:type="vs:ParamHTTP">
      <accessURL use="full"
        > http://myarchive.net/myTAP/auth-tables </accessURL>
      <securityMethod 
        standardID="http://www.w3.org/Protocols/HTTP/1.0/spec.html#BasicAA"/>
    </interface>
  </vosi:capability>
...
is described by \citep{std:VOSI}.
...
described in \citep{std:DALI11} if applicable. Timestamp values are usable if there
...
The RESPONSEFORMAT parameter is fully described in \citep{std:DALI11}. For 
backwards 
...
\subsubsection{RUNID}
The RUNID parameter is fully described in \citep{std:DALI11}.

\subsubsection{UPLOAD}
\label{sec:UPLOAD}

The UPLOAD parameter is described in \citep{std:DALI11}. Services should support 
the 
...
The \citep{std:DALI11} UPLOAD parameter supports both external resources and 
in-line 
...
The RESOURCE element must contain INFO element(s) as described in \citep{std:DALI11}.
...
Additional INFO elements after the results may also be included as specified in \citep{std:DALI11}.
...
format where possible; see DALI \citep{std:DALI11} for details. 
...
VOTable described in \citep{std:DALI11}. These rules apply to input tables 
...
in \citep{std:DALI11}. The behaviour of such structured values in queries depends 
...
ucd & char & * & null & false \\
indexed & int & 1 & null & true \\
principal & int & 1 & null & true \\
std & int & 1 & null & true \\
...
implementors should use standard values such as those defined in DALI \citep{std:DALI11} before 
...
section section~\ref{sec:tap-async}. TAP services may implement \citep{std:UWS} or
a later version. 
...
finishes. If the service implements \citep{std:UWS11} or later, a blocking call
...
\citep{std:DALI11}). There is no mechanism to replace or remove a parameter in a 
PENDING job.
...
As described in \citep{std:DALI11}, clients must be prepared to follow such 
...
Removed some VOTable content and reference DALI. Explicitly relaxed datatype and
arraysize used in TAP\_SCHEMA tables. Fixed various cross-references and typos. 

...
this authentication method uses the HTTP protocol to implement it's challenge-response and thus cannot co-exist with other methods (including anonymous); such a service would only be able to describe that single method. However, the SSO standard recommends that usernme-password authentication (including BasicAA) be used to login
and obtain another type of credential (e.g. a cookie) and that services should not directly use username-password
for authentication, so this limitation in TAP services should not be a problem.
